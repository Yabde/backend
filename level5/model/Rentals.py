...
class Rentals:
    ...
    def __init__(self, attributes_dict, data):
        ...
        :param data: DataBase Object to search the Owner's desired Car & Options list
        """
        ...
        self.options = data.get_item_by_name_and_id(
            "Options", self.id, first_match=False)
    ...
    def get_price_fee(self, car_owner):
        ...
        if n_days > 10:
            ...
            price += price_per_day * round(1 + 3*0.9 + (n_days-4)*0.7, 3)
        elif n_days > 1:
            price += price_per_day * round(1 + (n_days-1)*0.9, 3)
        else:
            price += price_per_day
        price = int(price)
    ...
    def get_final_output(self, data):
        ...
        n_days = self.n_days
        commission = Commission.compute_commission(n_days, price, self.options)
